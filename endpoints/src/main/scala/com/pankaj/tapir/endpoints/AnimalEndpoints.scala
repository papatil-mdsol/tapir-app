package com.pankaj.tapir.endpoints

import io.circe.generic.auto.{exportDecoder, exportEncoder}
import sttp.model.StatusCode
import sttp.tapir.generic.auto.schemaForCaseClass
import sttp.tapir.json.circe.jsonBody
import sttp.tapir.{Endpoint, endpoint, path, query, statusCode, stringBody}
...
object AnimalEndpoints {
  ...
  val kittenPost: Endpoint[Unit, Kitten, (StatusCode, ErrorResponse), (StatusCode, Kitten), Any] = endpoint
    .post
    .in("kitten")
    .in(jsonBody[Kitten])
    .errorOut(statusCode)
